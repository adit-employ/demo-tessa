<template>
           <div>
               <v-card :loading="events.loading">
                   ...
                   <v-card-actions>
                       <v-spacer></v-spacer>
                       <v-btn color="primary" outlined @click="$router.push({ name: `grade`})" :disabled="events.loading">
                           Back
                       </v-btn>
       ...
       <script>
           // Services
           import { createGrade, getGradeDetail, updateGrade } from "@/services/grade.js";
           ...
           export default {
               ...
               methods:{
                   doGetGradeDetail() {
                       ...
                       getGradeDetail(this.$route.params.id)
                           .then((response) => {
                               this.events.loading = false;
                               this.form = response;
                           }).catch((error) => {
                   ...
                   async doCreate() {
                       this.events.loading = true;
                       await createGrade(this.form_submit)
                           .then(() => {
                                setTimeout(() => {
