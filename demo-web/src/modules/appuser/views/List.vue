<template>
           <v-card>
               ...
               <v-data-table :headers="headers"
                             ...
                             class="elevation-1">
                               <!-- <template v-slot:[`item.appRole`]="{ item }">
                                   <ol>
                                       <li v-for="role in item.appRole" :key="role.id">
                                           {{ role.name }}
                                       </li>
                                   </ol>
                               </template> -->
                               ...
                               <template v-slot:[`item.action`]="{ item }">
                                   <v-tooltip bottom>
                                       <template v-slot:activator="{ on }">
                                           <v-btn icon v-on="on" @click="$router.push({ name: 'update-user', params: { id: item.id } })">
                                               <v-icon>
                                                   mdi-pencil
                                               </v-icon>
                                           </v-btn>
       ...
       <script>
           // Services
           import { getUserAll } from "@/services/app-users.js";
               ...
               data() {
                   return {
                       ...
                       headers: [
                           ...
                           { text: 'Role', value: 'appRole.name', sortable: true},
