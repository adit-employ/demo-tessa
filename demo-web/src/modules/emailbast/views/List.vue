        ...
        <v-data-table :headers="headers"
                      ...
                      class="elevation-1">
                        ...
                        <template v-slot:[`item.action`]="{ item }">
                            <v-tooltip bottom>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" @click="$router.push({ name: 'update-emailbast', params: { id: item.id } })">
                                        <v-icon>
                                            mdi-pencil
                                        </v-icon>
                                    </v-btn>
...
<script>
    ...
    export default {
        name: "VueListEmailBAST",
        data() {
            return {
                search: null,
                loading: true,                
                headers: [
                    { text: 'Email BAST', value: 'email', sortable: true},
