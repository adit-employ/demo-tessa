<template>
           <div>
               <v-card :loading="events.loading">
                   ...
                   <v-card-actions>
                       <v-spacer></v-spacer>
                       <v-btn color="primary" outlined @click="$router.push({ name: `brand`})" :disabled="events.loading">
                           Back
                       </v-btn>
       ...
       <script>
           // Services
           import { createBrand, getBrandDetail, updateBrand } from "@/services/brand.js";
           ...
           export default {
               ...
               data() {
                   return {
                       ...
                       rules: {
                                 name: [ v => !!v || 'Name Brand is required' ],
                       },
                       form_submit: {},
                       loading: false,
                       form_mode: null,
               ...
               methods:{
                   doGetBrandDetail() {
                       ...
                       getBrandDetail(this.$route.params.id)
                           .then((response) => {
                               this.events.loading = false;
                               this.form = response;
                           }).catch((error) => {
                   ...
                   async doCreate() {
                       this.events.loading = true;
                       await createBrand(this.form_submit)
                           .then(() => {
                                setTimeout(() => {
