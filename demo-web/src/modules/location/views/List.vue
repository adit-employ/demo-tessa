        ...
        <v-data-table :headers="headers"
                      ...
                      class="elevation-1">
                        ...
                        <template v-slot:[`item.action`]="{ item }">
                            <v-tooltip bottom>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" @click="$router.push({ name: 'update-location', params: { id: item.id } })">
                                        <v-icon>
                                            mdi-pencil
                                        </v-icon>
                                    </v-btn>
...
<script>
    // Services
    import { getLocationAll } from "@/services/location.js";
    ...
    export default {
        name: "VueListLocation",
        data() {
            return {
                search: null,
                loading: true,                
                headers: [
                    { text: 'Name Location', value: 'name', sortable: true},
