<template>
    <div>
        <v-card :loading="events.loading">
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="primary" outlined @click="$router.push({ name: `technology`})" :disabled="events.loading">
                    Back
                </v-btn>
...
<script>
    // Services
    import { createTechnology, getTechnologyDetail, updateTechnology } from "@/services/technology.js";

    //utils
    import errorMessage from "@/utils/notify-error";

    export default {
        name: "VueTechnologyForm",
        data() {
            return {
                form: {
                            name: null,
                ...
                rules: {
                            name: [ v => !!v || 'Name Technology is required' ],
                },
                form_submit: {},
                loading: false,
                form_mode: null,
        ...
        methods:{
            doGetTechnologyDetail() {
                this.events.loading = true;
                getTechnologyDetail(this.$route.params.id)
                    .then((response) => {
                        this.events.loading = false;
                        this.form = response;
                    }).catch((error) => {
            ...
            async doCreate() {
                this.events.loading = true;
                await createTechnology(this.form_submit)
                    .then(() => {
                        setTimeout(() => {
            ...
            saveSuccess() {
                ...
                this.$notify({
                    title: "Successful",
                    content: "Technology already saved!",
                    color: "rgb(0, 200, 83)",
                });
