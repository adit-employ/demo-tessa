    ...
    <v-card>
        <v-card-title>
            ...
            <v-tooltip bottom>
                ...
                <span>Add Assessment Filtering Test</span>
            </v-tooltip>
            <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                    <v-btn v-on="on"
        ...
        <v-data-table :headers="headers"
                      ...
                      class="elevation-1">
                        ...
                        <template v-slot:[`item.action`]="{ item }">
                            <v-tooltip bottom>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" @click="$router.push({ name: 'update-assessmentfilteringtest', params: { id: item.id } })">
                                        <v-icon>
                                            mdi-pencil
                                        </v-icon>
                                    </v-btn>
                                </template>
                                <span>Edit Assessment Filtering Test</span>
                            </v-tooltip>
                        </template>
        </v-data-table>
    </v-card>
...
<script>
    // Services
    import { getAssessmentFilteringTestAll } from "@/services/assessmentfilteringtest.js";
    ...
    export default {
        name: "VueListAssessmentFilteringTest",
        data() {
            return {
                search: null,
                loading: true,                
