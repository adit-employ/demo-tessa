<template>
           <div>
               <v-card :loading="events.loading">
                   ...
                   <v-card-actions>
                       <v-spacer></v-spacer>
                       <v-btn color="primary" outlined @click="$router.push({ name: `university`})" :disabled="events.loading">
                           Back
                       </v-btn>
       ...
       <script>
           // Services
           import { createUniversity, getUniversityDetail, updateUniversity } from "@/services/university.js";
       
           //utils
           import errorMessage from "@/utils/notify-error";
       
           export default {
               name: "VueUniversityForm",
               data() {
                   return {
                       form: {
                                 name: null,
                       ...
                       rules: {
                                 name: [ v => !!v || 'Name University is required' ],
                       },
                       form_submit: {},
                       loading: false,
                       form_mode: null,
               ...
               methods:{
                   doGetUniversityDetail() {
                       this.events.loading = true;
                       getUniversityDetail(this.$route.params.id)
                           .then((response) => {
                               this.events.loading = false;
                               this.form = response;
                           }).catch((error) => {
                   ...
                   async doCreate() {
                       this.events.loading = true;
                       await createUniversity(this.form_submit)
                           .then(() => {
                   ...
                   saveSuccess() {
                       ...
                       this.$notify({
                           title: "Successful",
                           content: "University already saved!",
                           color: "rgb(0, 200, 83)",
                       });
