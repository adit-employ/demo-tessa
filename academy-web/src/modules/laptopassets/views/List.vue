        ...
        <v-data-table :headers="headers"
                      ...
                      class="elevation-1">
                        ...
                        <template v-slot:[`item.action`]="{ item }">
                            <v-tooltip bottom>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" @click="$router.push({ name: 'update-laptopassets', params: { id: item.id } })">
                                        <v-icon>
                                            mdi-pencil
                                        </v-icon>
                                    </v-btn>
                                </template>
                                <span>Edit Laptop Assets</span>
                            </v-tooltip>
                        </template>
        </v-data-table>
    </v-card>
...
<script>
    // Services
    import { getLaptopAssetsAll } from "@/services/laptopassets";
    ...
    export default {
        name: "VueListLaptopAssets",
        data() {
            return {
                search: null,
                loading: true,                
                headers: [
                    { text: 'Laptop Bag Number', value: 'laptopBagNumber', sortable: true},
                    { text: 'Asset Number', value: 'assetNumber', sortable: true},
                    { text: 'Type / Series', value: 'type', sortable: true},
                    { text: 'Location', value: 'location.name', sortable: true},
                    { text: 'Status Laptop', value: 'status', sortable: true, align: "center"},
            ...
            doGetData() {
                ...
                getLaptopAssetsAll()
                    .then(response => {        
                        ...
                        console.log('Assets => this.list_data.data : ', this.list_data.data);
                    })
                    .catch(error => {
                        this.loading = false
                        errorMessage(error);
