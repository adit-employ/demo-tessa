        ...
        <v-data-table :headers="headers"
                      ...
                      class="elevation-1">
                        ...
                        <template v-slot:[`item.action`]="{ item }">
                            <v-tooltip bottom>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" @click="$router.push({ name: 'update-university', params: { id: item.id } })">
                                        <v-icon>
                                            mdi-pencil
                                        </v-icon>
                                    </v-btn>
...
<script>
    // Services
    import { getUniversityAll } from "@/services/university.js";
    ...
    export default {
        name: "VueListUniversity",
        data() {
            return {
                search: null,
                loading: true,                
                headers: [
                    { text: 'Name University', value: 'name', sortable: true},
