        ...
        <v-data-table :headers="headers"
                      ...
                      class="elevation-1">
                        ...
                        <template v-slot:[`item.action`]="{ item }">
                            <v-tooltip bottom >
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" @click="$router.push({ name: 'detail-bootcampassessmenttrainer', params: { id: item.id } })">
                                        <v-icon>mdi mdi-file-document-arrow-right</v-icon>
                                    </v-btn>
                                </template>
                                <span>Detail Bootcamp Assessment Trainer</span>
                            </v-tooltip>
                        </template>
        </v-data-table>
    </v-card>
...
<script>
    // Services
    import { getBootcampAssessmentTrainerAll } from "@/services/bootcampassessmenttrainer.js";
    ...
    export default {
        name: "VueListBootcampAssessmentTrainer",
        data() {
            return {
                search: null,
                loading: true,                
        ...
        methods: {
            ...
            doGetData() {
                this.loading = true
                getBootcampAssessmentTrainerAll(JSON.parse(localStorage.getItem("user")).id)//userId
                    .then(response => {
                        console.log('Page Assessment Trainer => List bootcamp : ', response.items)
                        this.loading = false;
